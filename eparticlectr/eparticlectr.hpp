...
public:
	...
	struct [[eosio::table]] voteextra_t {
		...
		uint32_t timestamp; // UNIX timestamp of the vote
		uint64_t stake_id; // the id of the stake generated by the vote. used for slashing
		string memo; // optional memo to pass thru logging actions. primarily used by proxy contracts to identify sub-permisisons responsible for an edit. Size limit is MAX_MEMO_SIZE
		string proxied_for;
		string extra_note;
		uint64_t primary_key()const { return id; }
		fixed_bytes<32> hash_proxied_for () const { return sha256_proxied_for(proxied_for); }
	...
	struct [[eosio::table]] editproposal {
		...
		string slug; // article slug. Size limit is MAX_SLUG_SIZE
		ipfshash_t ipfs_hash; // IPFS hash of the proposed new version
		int64_t group_id;
		string lang_code; // ISO 639-1 language code. Size limit is MAX_LANG_CODE_SIZE
